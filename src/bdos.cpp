...
static void WriteFCB(FCB &fcb, Z80_CPU &cpu) {
  ...
  BYTE buf[37];
  fcb.GetBlock(buf);
  cpu.SetMemory((UINT8 *)buf, 37, DE);
}

void EMU_BDOS::SetConsole(std::istream *in, std::ostream *out) {
  cin = in;
  cout = out;
}

std::iostream *OnCreateFile(FCB fcb) { return NULL; }

bool EMU_BDOS::DoProcess(Z80_CPU &z80) {
  BYTE buf[37];
  ...
  case 0x21: // Random Read (CP/M compatible)
    ...
    WriteFCB(fcb, z80);
    z80.SetMemory((UINT8 *)tmp_buf, 128, DTA);
    z80.SetRegs8(REGID_A, 0);
    return true;
  ...
  case 0x22: // Random Write (CP/M compatible)
    ...
    z80.GetMemory((UINT8 *)tmp_buf, 128, DTA);
