...
MGSC::MGSC() { errmsg[0] = '\0'; }

MGSC::~MGSC() {}

bool MGSC::LoadCompiler(std::istream *is) {
  is->read((char *)mgsc, 0x8000);
  mgsc_size = is->gcount();
  return (mgsc_size > 0);
}

void MGSC::SetConsole(std::istream *in, std::ostream *out) {
  bdos.SetConsole(in, out);
}

bool MGSC::Compile(std::iostream *mml, std::iostream *mgs, int option) {
  ...
  ram.SetImage((UINT8 *)mgsc, 0x100, mgsc_size);
  cpu.Reset();
  ...
  while (1) {
    ...
    } else if (cpu.GetPC() == 0x0038) // H.TIMI
    {
      ...
      if (bdos.DoProcess(cpu)) {
        ...
        sprintf(errmsg, "Unsupported BDOS call : Function %02xH",
                cpu.GetRegs8(REGID_C));
        return false;
      }
    } else {
...
const char *MGSC::GetErrorMessage() { return errmsg; }
