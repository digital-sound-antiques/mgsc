...
The compression algorithm is a sort of [dictionay coder](https://en.wikipedia.org/wiki/Dictionary_coder), similar to a little endian version of [lze](http://gorry.haun.org/pw/?lze) (based on [lzexe](https://bellard.org/lzexe.html)). An overview of compression format can be found [here](./lzexe.jpg). Uncompress function is here: [uncompress.ts](https://github.com/digital-sound-antiques/mgsrc-js/blob/master/src/uncompress.ts).
